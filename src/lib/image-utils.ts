...
export async function processUploadedImage(absPath: string): Promise<{ newPath: string; thumbPath: string } | null> {
  ...
  // New upload: move to date folder
  ...
  if (fs.existsSync(targetPath)) {
    ...
    if (existingSize === newSize && fileHash(absPath) === fileHash(targetPath)) {
      return { newPath: `${dateDir}/${finalName}`, thumbPath: `${dateDir}/${thumbName}` };
    }
  ...
  try {
    ...
    return { newPath: `${dateDir}/${finalName}`, thumbPath: `${dateDir}/${finalName.replace(ext, `_thumb${ext === '.png' ? '.jpg' : ext}`)}` };
  } catch {
    return null;
  }
}
